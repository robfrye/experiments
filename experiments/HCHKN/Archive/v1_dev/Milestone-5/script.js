...
function initGame() {
    ...
    // Spawn initial collectibles for testing
...
function update(deltaTime) {
    ...
    if (gameStats.isGameOver) {
        return; // Stop updating game logic if game is over
    }
...
// Draw dumpling collectible
function drawDumpling(x, y, collectible) {
    // Dumpling wrapper (light brown)
    ctx.fillStyle = '#deb887';
    ctx.fillRect(x + 4, y + 8, 24, 16);
    
    // Dumpling pleats (darker brown)
    ctx.fillStyle = '#d2691e';
    for (let i = 0; i < 4; i++) {
        ctx.fillRect(x + 6 + i * 5, y + 6, 2, 8);
    }
    
    // Dumpling filling (visible through wrapper)
    ctx.fillStyle = '#8b4513';
    ctx.fillRect(x + 8, y + 12, 16, 8);
    
    // Steam effect (simple animation)
    if (collectible.animationFrame % 2 === 0) {
        ctx.fillStyle = '#ffffff';
        ctx.globalAlpha = 0.5;
        ctx.fillRect(x + 12, y - 5, 2, 8);
        ctx.fillRect(x + 16, y - 8, 2, 8);
        ctx.fillRect(x + 20, y - 3, 2, 8);
        ctx.globalAlpha = 1.0;
    }
...
function drawNoodleSoup(x, y, collectible) {
    ...
    // Soup broth (golden brown)
    ctx.fillStyle = '#cd853f';
    ctx.fillRect(x + 4, y + 14, 24, 12);
    
    // Noodles (yellow strands)
    ctx.fillStyle = '#ffd700';
    for (let i = 0; i < 6; i++) {
        const noodleOffset = Math.sin(collectible.bobOffset + i) * 2;
        ctx.fillRect(x + 6 + i * 3, y + 16 + noodleOffset, 2, 8);
    }
    
    // Chopsticks
    ctx.fillStyle = '#8b4513';
    ctx.fillRect(x + 20, y + 4, 2, 20);
    ctx.fillRect(x + 24, y + 6, 2, 18);
    
    // Steam effect (animated)
    if (collectible.animationFrame % 3 === 0) {
        ctx.fillStyle = '#ffffff';
        ctx.globalAlpha = 0.6;
        ctx.fillRect(x + 8, y - 2, 2, 10);
        ctx.fillRect(x + 14, y - 5, 2, 10);
        ctx.fillRect(x + 20, y - 1, 2, 8);
        ctx.globalAlpha = 1.0;
    }
